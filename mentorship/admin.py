...
from .filters import MenteeListFilter, MentorListFilter
...
def export_as_csv(self, request, queryset):
    ...
    for obj in queryset:
        ...
        if isinstance(obj, MenteeProfile):
            qualifications = [
                (q.name, q.education_level, q.grade, q.predicted)
                for q in MenteeQualification.objects.filter(profile=obj)
            ]
...
class MentorAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        (
            "Meta",
            {"fields": ["date_joined", "hear_about_us", "is_active", "tcs_consent"]},
        ),
    ]
...
def save_matches(request, matches, ct):
    for k, v in matches.items():
        ...
        mentor = MentorProfile.objects.get(id=v)
...
class MenteeAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        ("Meta", {"fields": ["date_joined", "hear_about_us", "tcs_consent"]}),
    ]
    ...
    def export_matches_info(self, request, queryset):
        ...
        column_names = [
            ...
            "mentee.year_applied",
            "mentor.year_applied",
            "mentee.area_of_support",
            "mentor.area_of_support",
            "mentee.interview_experience",
            "mentor.interview_experience",
            "mentee.entrance_exam_experience",
            "mentor.entrance_exam_experience",
        ]
        response = HttpResponse(content_type="text/csv")
    ...
    def assign_mentor(self, request, queryset):
        ct = ContentType.objects.get_for_model(queryset.model)
        available_mentors = MentorProfile.objects.filter(menteeprofile__isnull=True)
        available_medicine_mentors = available_mentors.filter(
            occupation__startswith="M"
        )
        available_dentistry_mentors = available_mentors.difference(
            available_medicine_mentors
        )
        unmatched_mentees = queryset
        unmatched_medicine_mentees = unmatched_mentees.filter(course__startswith="M")
        unmatched_dentistry_mentees = unmatched_mentees.difference(
            unmatched_medicine_mentees
        )
        medicine_matches, medicine_prefs = generate_matches(
            available_medicine_mentors, unmatched_medicine_mentees
        )
        dentistry_matches, dentistry_pref = generate_matches(
            available_dentistry_mentors, unmatched_dentistry_mentees
        )

        validate_stable_matching(medicine_matches, medicine_prefs)
        print_matching_statistics(medicine_matches, medicine_prefs, "medicine")

        validate_stable_matching(dentistry_matches, dentistry_pref)
        print_matching_statistics(dentistry_matches, dentistry_pref, "dentistry")
...
class LogEntryAdmin(admin.ModelAdmin):
    ...
    list_display = ("action_time", "user", "action_message", "content_type")
    list_filter = (
        "action_flag",
        "content_type",
    )
